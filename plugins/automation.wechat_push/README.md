...
| `system.warning` | 系统告警 |
| `notification.test` | 测试通知（始终处理，不受订阅过滤） |

## 通知测试

配置完成后，可以通过两种方式发送测试通知验证推送是否生效：

### 方式一：调用 `test_notification()` 方法

插件暴露了 `test_notification(config=None)` 方法：

- **不传参数**：使用当前运行时配置发送测试消息
- **传入 config**：使用临时配置测试（不影响运行时配置）

测试消息内容包含当前推送地址、UID（脱敏）、订阅事件、超时和重试设置，便于确认配置。

### 方式二：触发 `notification.test` 事件

通过 `handle()` 发送 `event_type: "notification.test"` 事件，效果等同于调用 `test_notification()`。该事件类型始终处理，不受订阅列表限制。
...
```
标题：[T3FAP] 测试通知
正文：这是一条来自 T3FAP 微信推送插件的测试消息。
如果您在微信中收到了这条消息，说明推送配置已正确生效。

推送地址：https://wechat.powerautomate.top/api/push
UID：02cbf1a7*******
订阅事件：task.completed, task.failed
超时时间：10 秒
重试次数：2 次
```
...
| 版本 | 日期 | 说明 |
| --- | --- | --- |
| 1.1.0 | 2026-07-03 | 新增 `test_notification()` 方法和 `notification.test` 事件支持，支持通知测试验证 |
